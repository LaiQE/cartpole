...
onet = CartPoleControl.CratpoleControl(chrom[0],chrom[1],chrom[2],chrom[3],chrom[4],chrom[5],chrom[6],100,10000,1,True)
x = np.arange(len(onet))
plt.plot(x, onet[:, 0],'r', label='x')
# plt.plot(x, onet[:, 1],'g', label='v')
plt.plot(x, onet[:, 2],'b', label='a')
# plt.plot(x, onet[:, 3],'p', label='av')
plt.xlabel("time")
plt.ylabel("function value")
plt.title("PID algorithm for cartplole")
plt.legend()
plt.show()
