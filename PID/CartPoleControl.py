...
def CratpoleControl(p_x, i_x, d_x, p_theta, i_theta, d_theta, lamb, episode, step_limit, isprint, onestep=False):
	tt = []
	if onestep:
		episode = 1
		onet = []
	for i_episode in range(episode):
		...
		for t in range(step_limit):
		    ...
		    theta_t2 = theta_t1
		    theta_t1 = theta_t
		    theta_t = observation[2]
		    if onestep:
		        onet.append(observation)
		    if done:
		    	if isprint:
		...
		if isprint and t==step_limit:
			print("Episode finished after {} timesteps".format(t+1))
			tt.append(t) 
	return tt if not onestep else np.array(onet)
